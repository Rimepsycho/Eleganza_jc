...
// 按照順序排列//軟刪除後還在資料庫
$sqlSorted = "SELECT ROW_NUMBER() OVER (ORDER BY user_id) AS DisplayID, user_id
        FROM users
        WHERE valid=1";
$resultSorted = $conn->query($sqlSorted);

$sortedUsers = $resultSorted->fetch_all(MYSQLI_ASSOC);

$displayId = 0;
foreach ($sortedUsers as $sortedUser) {
    if ($sortedUser['user_id'] == $id) {
        $displayId = $sortedUser['DisplayID'];
        break;
    }
}
